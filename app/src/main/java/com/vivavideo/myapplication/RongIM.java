...
import com.vivavideo.myapplication.imageloader.core.ImageLoader;
import com.vivavideo.myapplication.imageloader.core.assist.FailReason;
import com.vivavideo.myapplication.imageloader.core.listener.ImageLoadingListener;
import com.vivavideo.myapplication.imageloader.core.listener.ImageLoadingProgressListener;
...
import com.vivavideo.myapplication.model.GroupUserInfo;
import com.vivavideo.myapplication.model.UIConversation;
import com.vivavideo.myapplication.notification.MessageCounter;
import com.vivavideo.myapplication.notification.MessageNotificationManager;
import com.vivavideo.myapplication.provider.IContainerItemProvider;
import com.vivavideo.myapplication.provider.InputProvider;
import com.vivavideo.myapplication.provider.TextMessageItemProvider;
import com.vivavideo.myapplication.userInfoCache.RongUserInfoManager;
import com.vivavideo.myapplication.utils.CommonUtils;
import com.vivavideo.myapplication.utils.SystemUtils;
import com.vivavideo.myapplication.widget.RongUserCacheListener;
...
public class RongIM {
     ...
     * @param context 应用上下文。
     */
    private RongIM(Context context) {
    ...
    public static void init(Context context) {
        ...
        mCurrentProcessName = current;
        mContext = context;
        sRongIM = new RongIM(context);
        RongContext.init(context);
        String appKey = RongContext.getInstance().getAppKey();
        ...
        RongUserInfoManager.init(appKey, new RongUserCacheListener());
...
//        registerMessageTemplate(new UnknownMessageItemProvider());
    ...
    public static void init(Context context, String appKey) {
        ...
        RongContext.init(context);
        RongContext.getInstance().saveAppKey(appKey);
        init(context);
    }
    ...
    public static void registerMessageType(Class<? extends MessageContent> messageContentClass) {
        ...
        if (RongContext.getInstance() == null) {
            RLog.e(TAG, "registerMessageType RongCloud SDK not init");
            return;
        }
    ...
    public static void registerMessageTemplate(IContainerItemProvider.MessageProvider provider) {
        ...
        if (RongContext.getInstance() == null) {
            RLog.e(TAG, "registerMessageTemplate RongCloud SDK not init");
            return;
        }
    ...
    public static RongIM connect(String token, final RongIMClient.ConnectCallback callback) {
        ...
        RongIMClient.connect(token, new RongIMClient.ConnectCallback() {
            ...
            public void onSuccess(String userId) {
                ...
                RongUserInfoManager.connectDB(mContext, RongIMClient.getInstance().getCurrentUserId());
            }
                ...
                if (msgTag != null && (msgTag.flag() == MessageTag.ISCOUNTED || msgTag.flag() == MessageTag.ISPERSISTED)) {
                    ...
                    if (message.getConversationType() == Conversation.ConversationType.PRIVATE) {
                        ...
                        if (className.equals("io.rong.voipkit.message.VoIPCallMessage") || className.equals("io.rong.voipkit.message.VoIPAcceptMessage") || className.equals("io.rong.voipkit.message.VoIPFinishMessage")) {
                            RongContext.getInstance().getEventBus().post(new Event.OnReceiveVoIPMessageEvent(message, left));
                        }
                    } else if (message.getConversationType().equals(Conversation.ConversationType.CUSTOMER_SERVICE)) {
                        RongContext.getInstance().getEventBus().post(new Event.OnCustomServiceEvent(message));
                    }
                }
     ...
     *                              例如：supportedConversation.put(Conversation.ConversationType.PRIVATE.getName(),
     *                              false) 非聚合式显示 private 类型的会话。
     */
    ...
    public static void setConversationBehaviorListener(ConversationBehaviorListener listener) {
        ...
        if (RongContext.getInstance() == null) {
            RLog.e(TAG, "RongIM RongCloud hasn't been init!!!!");
            return;
        }
    ...
    /**
     * 创建讨论组会话并进入会话界面。
     * <p>该方法会同时根据传入的 userId 创建讨论组，并通过{@link io.rong.imlib.RongIMClient.CreateDiscussionCallback}
     * 返回讨论组创建结果。</p>
     *
     * @param context       应用上下文。
     * @param targetUserIds 要与之聊天的讨论组用户 Id 列表。
     ...
     * 如果聊天室不存在，则返回错误 {@link io.rong.imlib.RongIMClient.ErrorCode#RC_CHATROOM_NOT_EXIST}，并且会话界面会弹出
     * warning.
     * </p>
     *
     * @param context          应用上下文。
     ...
     * 设置后，当 sdk 界面展示用户信息时，会回调 {@link (String)}  io.rong.imkit.RongIM.UserInfoProvider#getUserInfo
     ...
     * </p>
     *
     * @param userInfoProvider 用户信息提供者 {@link }。vio.rong.imkit.RongIM.UserInfoProvider
...
//    }
    ...
    public void setOnReceiveUnreadCountChangedListener(final OnReceiveUnreadCountChangedListener listener, Conversation.ConversationType... conversationTypes) {
        ...
        if (listener != null && conversationTypes != null && conversationTypes.length > 0) {
            MessageCounter.Counter mCounter = new MessageCounter.Counter(ConversationTypeFilter.obtain(conversationTypes)) {
                @Override
                public void onMessageIncreased(int count) {
    ...
    public static void setPrimaryInputProvider(InputProvider.MainInputProvider provider) {
        ...
        if (RongContext.getInstance() == null) {
            RLog.e(TAG, "setPrimaryInputProvider RongCloud SDK not init");
            return;
        }
    ...
    public static void setSecondaryInputProvider(InputProvider.MainInputProvider provider) {
        ...
        if (RongContext.getInstance() == null) {
            RLog.e(TAG, "setSecondaryInputProvider RongCloud SDK not init");
            return;
        }
    ...
    public static void resetInputExtensionProvider(Conversation.ConversationType conversationType, InputProvider.ExtendProvider[] providers) {
        ...
        if (RongContext.getInstance() == null) {
            RLog.e(TAG, "registerInputProvider RongCloud SDK not init");
            return;
        }
    ...
    public static void addInputExtensionProvider(Conversation.ConversationType conversationType, InputProvider.ExtendProvider[] providers) {
        ...
        if (mCurrentProcessName != null && !mCurrentProcessName.equals(mContext.getPackageName())) {
            RLog.e(TAG, "Rong SDK should not be initialized at subprocess");
            return;
        }
        ...
        RongContext.getInstance().addInputExtentionProvider(conversationType, providers);
    }
    ...
    public void registerConversationTemplate(IContainerItemProvider.ConversationProvider provider) {
        ...
        if (RongContext.getInstance() == null) {
            RLog.e(TAG, "registerConversationTemplate RongCloud SDK not init");
            return;
        }
    ...
    public void setMaxVoiceDurationg(int sec) {
        RongContext.getInstance().getVoiceInputProvider().setMaxVoiceDuration(sec);
    }
     ...
     * @param callback 连接回调。
     */
    public void reconnect(RongIMClient.ConnectCallback callback) {
     ...
     * @param targetId 目标 Id。根据不同的 conversationType，可能是用户 Id、讨论组 Id、群组 Id 或聊天室 Id。
     * @return 会话信息。
     * @deprecated 该方法废弃，请使用{@link #getConversation(Conversation.ConversationType, String,
     ...
     * @param targetId 目标 Id。根据不同的 conversationType，可能是用户 Id、讨论组 Id、群组 Id 或聊天室 Id。
     * @return 是否移除成功。
     * @deprecated 此方法废弃，请使用{@link #removeConversation(Conversation.ConversationType, String,
     * RongIMClient.ResultCallback)}
     */
    @Deprecated
     ...
     * @param isTop            是否置顶。
     * @return 是否设置成功。
     * @deprecated 此方法废弃，请使用{@link #setConversationToTop(Conversation.ConversationType, String,
     * boolean, RongIMClient.ResultCallback)}。
     */
    @Deprecated
     ...
     * @param targetId         目标 Id。根据不同的 conversationType，可能是用户 Id、讨论组 Id、群组 Id。
     * @return 未读消息数。
     * @deprecated 此方法废弃，请使用{@link #getUnreadCount(Conversation.ConversationType, String,
     * RongIMClient.ResultCallback)}。
     */
    @Deprecated
     ...
     * @param conversationTypes 会话类型。
     * @return 未读消息数的回调。
     * @deprecated 此方法废弃，请使用{@link #getUnreadCount(RongIMClient.ResultCallback,
     * Conversation.ConversationType...)}。
     */
    @Deprecated
     ...
     * @deprecated 此方法废弃，请使用{@link #getLatestMessages(Conversation.ConversationType, String, int,
     * RongIMClient.ResultCallback)}。
     */
    @Deprecated
     ...
     * @deprecated 此方法废弃，请使用{@link #getHistoryMessages(Conversation.ConversationType, String, int,
     * int, RongIMClient.ResultCallback)}。
     */
    @Deprecated
     ...
     * @param count            要获取的消息数量。
     * @return 历史消息记录，按照时间顺序从新到旧排列。
     * @deprecated 此方法废弃，请使用{@link #getHistoryMessages(Conversation.ConversationType, String,
     * String, int, int, RongIMClient.ResultCallback)}。
     */
    @Deprecated
     ...
     * @param targetId         目标 Id。根据不同的 conversationType，可能是用户 Id、讨论组 Id、群组 Id。
     * @return 是否清空成功。
     * @deprecated 此方法废弃，请使用{@link #clearMessages(Conversation.ConversationType, String,
     * RongIMClient.ResultCallback)}。
     */
    @Deprecated
     ...
     * @param targetId         目标 Id。根据不同的 conversationType，可能是用户 Id、讨论组 Id、群组 Id。
     * @return 是否清空成功。
     * @deprecated 此方法废弃，请使用{@link #clearMessagesUnreadStatus(Conversation.ConversationType, String,
     * RongIMClient.ResultCallback)}。
     */
    @Deprecated
     ...
     * @return 是否设置成功。
     * @deprecated 此方法废弃，请使用{@link #setMessageReceivedStatus(int, Message.ReceivedStatus,
     * RongIMClient.ResultCallback)}。
     */
    @Deprecated
     ...
     * @return 是否设置成功。
     * @deprecated 此方法废弃，请使用{@link #setMessageSentStatus(int, Message.SentStatus,
     * RongIMClient.ResultCallback)}。
     */
    @Deprecated
     ...
     * @param targetId         目标 Id。根据不同的 conversationType，可能是用户 Id、讨论组 Id、群组 Id 或聊天室 Id。
     * @return 草稿的文字内容。
     * @deprecated 此方法废弃，请使用{@link #getTextMessageDraft(Conversation.ConversationType, String,
     * RongIMClient.ResultCallback)}。
     */
    @Deprecated
     ...
     * @return 是否保存成功。
     * @deprecated 此方法废弃，请使用{@link #saveTextMessageDraft(Conversation.ConversationType, String,
     * String, RongIMClient.ResultCallback)}。
     */
    @Deprecated
     ...
     * @param targetId         目标 Id。根据不同的 conversationType，可能是用户 Id、讨论组 Id、群组 Id 或聊天室 Id。
     * @return 是否清除成功。
     * @deprecated 此方法废弃，请使用{@link #clearTextMessageDraft(Conversation.ConversationType, String,
     * RongIMClient.ResultCallback)}。
     */
    @Deprecated
    ...
    public void setDiscussionName(final String discussionId, final String name, final RongIMClient.OperationCallback callback) {
        RongIMClient.getInstance().setDiscussionName(discussionId, name, new RongIMClient.OperationCallback() {
            ...
            public void onSuccess() {
                if (callback != null) {
                    RongContext.getInstance().getEventBus().post(new Discussion(discussionId, name));
                    callback.onSuccess();
                }
            }
        });
     ...
     *                     注解中设置{ProviderTag#showProgress()}值为false即可。
    ...
    public void insertMessage(Conversation.ConversationType type, String targetId, String senderUserId, MessageContent content, final RongIMClient.ResultCallback<Message> callback) {
        ...
        if (tag != null && (tag.flag() & MessageTag.ISPERSISTED) == MessageTag.ISPERSISTED) {
            ...
            RongIMClient.getInstance().insertMessage(type, targetId, senderUserId, content, new RongIMClient.ResultCallback<Message>() {
                @Override
     ...
     * @param content      消息内容。
     * @return Message
     * @deprecated 此方法废弃，请使用{@link #insertMessage(Conversation.ConversationType, String, String,
     * MessageContent, RongIMClient.ResultCallback)}。
     */
    @Deprecated
     ...
     * @param pushData    push 附加信息。如果设置该字段，用户在收到 push 消息时，能通过 {@link io.rong.push.notification.PushNotificationMessage#getPushData()}
     *                    方法获取。
...
//                mContext.getEventBus().post(new Event.OnMessageSendErrorEvent(result.t, errorCode));
     ...
     * {@link RongIMClient.uploadImageStatusListener#success()} 更新成功状态
     ...
     * @param pushData    push 附加信息。如果设置该字段，用户在收到 push 消息时，能通过 {@link io.rong.push.notification.PushNotificationMessage#getPushData()}
     *                    方法获取。
     * @param callback    发送消息的回调，回调中携带 {@link RongIMClient.uploadImageStatusListener}
     *                    对象，用户调用该对象中的方法更新状态。
    ...
    public void downloadMedia(String imageUrl, final RongIMClient.DownloadMediaCallback callback) {
        ...
        ImageLoader.getInstance().loadImage(imageUrl, null, null, new ImageLoadingListener() {
            ...
            public void onLoadingFailed(String imageUri, View view, FailReason failReason) {
                if (callback != null)
                    callback.onFail(RongIMClient.ErrorCode.RC_NET_UNAVAILABLE);
            }

            @Override
            public void onLoadingComplete(String imageUri, View view, Bitmap loadedImage) {
                if (callback != null)
                    callback.onCallback(imageUri);
            }
     ...
     * @param conversationTypes 会话类型。
     * @return 是否清空成功。
     * @deprecated 此方法已废弃，请使用{@link #clearConversations(RongIMClient.ResultCallback,
     * Conversation.ConversationType...)}。
     */
    @Deprecated
