...
public class EmojiMessageAdapter {
    ...
    public void getHistoryMessages(Conversation.ConversationType conversationType, String targetId, int oldestMessageId, int count, final RongIMClient.ResultCallback<List<UIMessage>> callback) {
        ...
        RongIM.getInstance().getHistoryMessages(conversationType, targetId, oldestMessageId, count, new RongIMClient.ResultCallback<List<Message>>() {
    ...
    public void getRemoteHistoryMessages(Conversation.ConversationType conversationType, String targetId, long dataTime, int count, final RongIMClient.ResultCallback<List<UIMessage>> callback) {
        RongIMClient.getInstance().getRemoteHistoryMessages(conversationType, targetId, dataTime, count, new RongIMClient.ResultCallback<List<Message>>() {
            @Override
            public void onSuccess(List<Message> messages) {
                if (callback != null) {
                    callback.onSuccess(emojiMessageToUIMessage(messages));
    ...
    public void getLatestMessages(Conversation.ConversationType conversationType, String targetId, int count, final RongIMClient.ResultCallback<List<UIMessage>> callback) {
        RongIM.getInstance().getLatestMessages(conversationType, targetId, count, new RongIMClient.ResultCallback<List<Message>>() {
