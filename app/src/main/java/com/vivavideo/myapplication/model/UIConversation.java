...
//        }
    ...
    public static UIConversation obtain(Message message, boolean gather) {
        ...
        if (message.getMessageDirection() == Message.MessageDirection.SEND) {
            tempUIConversation.setConversationSenderId(RongIM.getInstance().getCurrentUserId());
        } else {
    ...
    public SpannableStringBuilder buildConversationContent(UIConversation uiConversation) {
        ...
        ProviderTag providerTag = RongContext.getInstance().getMessageProviderTag(uiConversation.getMessageContent().getClass());
        IContainerItemProvider.MessageProvider messageProvider = RongContext.getInstance().getMessageTemplate(uiConversation.getMessageContent().getClass());
        ...
        messageData = messageProvider.getContentSummary(uiConversation.getMessageContent());
        isShowName = providerTag.showSummaryWithName();
        if (messageData == null) {
            builder.append("");
            return builder;
        }
        ...
        if (uiConversation.getMessageContent() instanceof VoiceMessage) {
            Conversation conv = RongIM.getInstance().getConversation(uiConversation.getConversationType(), uiConversation.getConversationTargetId());
            if (conv != null) {
                boolean isListened = conv.getReceivedStatus().isListened();
                if (conv.getSenderUserId().equals(RongIMClient.getInstance().getCurrentUserId()) || isListened) {
