package com.vivavideo.myapplication.utils;
...
public class AndroidEmoji {
    ...
    public static void init(Context context) {
        ...
        int[] codes = context.getResources().getIntArray(R.array.rc_emoji_code);
        TypedArray array = context.getResources().obtainTypedArray(R.array.rc_emoji_res);
    ...
    public static class EmojiImageSpan extends ReplacementSpan {
        ...
        private EmojiImageSpan(int codePoint) {
            if (sEmojiMap.containsKey(codePoint)) {
                mDrawable = RongContext.getInstance().getResources().getDrawable(sEmojiMap.get(codePoint).getRes());
    ...
    public static CharSequence ensure(String input) {
        ...
        for (int i = 0; i < chars.length; i++) {
            ...
            if (sEmojiMap.containsKey(codePoint)) {
                ssb.setSpan(new EmojiImageSpan(codePoint), isSurrogatePair ? i - 1 : i, i + 1, Spanned.SPAN_EXCLUSIVE_INCLUSIVE);
            }
        }
...
//        }
    ...
    public static void ensure(Spannable spannable) {
        ...
        for (int i = 0; i < chars.length; i++) {
            ...
            if (sEmojiMap.containsKey(codePoint)) {
                spannable.setSpan(new EmojiImageSpan(codePoint), isSurrogatePair ? i - 1 : i, i + 1, Spanned.SPAN_EXCLUSIVE_INCLUSIVE);
            }
        }
    }
